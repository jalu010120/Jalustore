// js/data.js

const deliveryData = [
    // Orderan Berjalan
    { slot: 1, name: 'Ney', order: 4340, send: 3230 },
    { slot: 1, name: 'Purili', order: 320, send: 300 },
    { slot: 1, name: 'Rain', order: 320, send: 300 },
    { slot: 1, name: 'Nayla', order: 530, send: 478 },
    { slot: 1, name: 'Aelia', order: 840, send: 400 },
    { slot: 1, name: 'Raine', order: 960, send: 380 },
    { slot: 1, name: 'Aya', order: 860, send: 260 },
    { slot: 1, name: 'Anna', order: 1080, send: 40 }, // Updated

    { slot: 2, name: 'Isa', order: 800, send: 140 },
    { slot: 2, name: 'Angel2', order: 600, send: 460, statusOverride: 'fail' },
    { slot: 2, name: 'Alip', order: 320, send: 280 },
    { slot: 2, name: 'Rani', order: 320, send: 280 },
    { slot: 2, name: 'Anggi', order: 320, send: 280 },
    { slot: 2, name: 'Autumn', order: 850, send: 230 },
    { slot: 2, name: 'Ave', order: 220, send: 60 },

    { slot: 3, name: 'Zee', order: 220, send: 80 },
    { slot: 3, name: 'Diyy', order: 320, send: 260 },
    { slot: 3, name: 'Aze', order: 570, send: 320, statusOverride: 'fail' },
    { slot: 3, name: 'Al', order: 320, send: 200, statusOverride: 'fail' },
    { slot: 3, name: 'Sena', order: 570, send: 340 },
    { slot: 3, name: 'Araba', order: 150, send: 132 },
    { slot: 3, name: 'Idnes', order: 320, send: 167 },
    { slot: 3, name: 'Luna', order: 800, send: 722, statusOverride: 'fail' },
    { slot: 3, name: 'Kai', order: 220, send: 60 },
    { slot: 3, name: 'Rei', order: 220, send: 60 },

    // Riwayat Orderan Selesai (Dipertahankan dari update sebelumnya)
    { slot: 'completed', name: 'Hira', order: 360, send: 377 },
    { slot: 'completed', name: 'Amanda', order: 80, send: 80 },
    { slot: 'completed', name: 'bulilisso', order: 80, send: 80 },
    { slot: 'completed', name: 'Tsabyy', order: 200, send: 200 },
    { slot: 'completed', name: 'Harbie', order: 200, send: 200 },
    { slot: 'completed', name: 'Fida', order: 100, send: 103 },
    { slot: 'completed', name: 'Vyan', order: 80, send: 83 },
    { slot: 'completed', name: 'Ayu', order: 1000, send: 1003 },
    { slot: 'completed', name: 'Anna', order: 600, send: 603 },
    { slot: 'completed', name: 'Zee', order: 150, send: 153 },
    { slot: 'completed', name: 'Nanda', order: 210, send: 213 },
    { slot: 'completed', name: 'Alvin', order: 210, send: 213 },
    { slot: 'completed', name: 'Ira', order: 150, send: 153 },
    { slot: 'completed', name: 'Kael', order: 120, send: 123 },
    { slot: 'completed', name: 'Xen', order: 210, send: 213 },
    { slot: 'completed', name: 'Lusi', order: 600, send: 603 },
    { slot: 'completed', name: 'Sea', order: 150, send: 157 }
];
